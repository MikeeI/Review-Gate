...
class ReviewGateServer:
    ...
    async def _handle_ingest_text(self, args: dict) -> list[TextContent]:
        ...
        if success:
            ...
            if user_input:
                ...
                result_message += f"📍 Source: {source}\n\n"
                result_message += (
                    f"No user response received within 2 minutes. The text content is noted but no additional processing occurred."
                )
    ...
    async def _wait_for_user_input(self, trigger_id: str, timeout: int = 120) -> str | None:
    ...
    async def run(self):
        ...
        async with stdio_server() as (read_stream, write_stream):
            ...
            server_task = asyncio.create_task(self.server.run(read_stream, write_stream, self.server.create_initialization_options()))
